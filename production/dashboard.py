...
ir_val = gsr_val = temp_val = att_val = med_val = 0.0

if df is not None:
    # Physio sensor: ir, gsr, tmp (rows with ir present)
    if "ir" in df.columns and df["ir"].notna().any():
        last_physio = df[df["ir"].notna()].iloc[-1]
...
# ─── Layout: Camera | VA Plane | Metrics ─────────────────────────────────────

col_cam, col_va, col_metrics = st.columns([2, 2, 1])
...
with col_va:
    ...
    if st.session_state.va_history:
        ...
        if status == "success":
            st.caption(f"Valence: **{v:.3f}**  |  Arousal: **{a:.3f}**")
    else:
...
with col_metrics:
    ...
    st.metric("HR (BPM)",    f"{hr_bpm:.1f}")
    st.metric("RMSSD (ms)",  f"{rmssd:.1f}")
